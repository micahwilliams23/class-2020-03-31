...
county %>% 
  summarise(avg_no_hdiploma = mean(hs))
...
```{r}
county %>% 
  summarize(correlation = cor(poverty,
                              less_than_hs))
```
...
```{r}
county_samples <- rep_sample_n(county, size = 3129, replace = T, reps = 1000) %>% 
  group_by(replicate) %>%
  nest() %>%
  mutate(distinct = map_dbl(data, ~n_distinct(.)))

glimpse(county_samples)
```
